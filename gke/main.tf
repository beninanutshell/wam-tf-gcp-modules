...
locals {
  ...
  cluster_name                 = "${var.cluster_name}-kcl-${var.env}-${var.location}"
...
resource "google_container_node_pool" "node_pool" {
  ...
  name = "${var.cluster_name}-npl-${var.env}-${var.location}-${lookup(var.node_pools[count.index], "name", format("%03d", count.index + 1))}"
